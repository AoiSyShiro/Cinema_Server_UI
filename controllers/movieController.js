...
const getMovies = async (req, res) => {
  try {
    const movies = await Movie.find(); // Lấy danh sách tất cả phim từ cơ sở dữ liệu
    ...
    const moviesWithCategories = movies.map(movie => {
      ...
      return {
        ...movie.toObject(), // Chuyển đổi phim thành object_id thuần
        category: category ? category.name : null  // Thêm tên thể loại vào thông tin phim
      };
    });

    res.json(moviesWithCategories); // Trả về danh sách phim kèm thông tin thể loại
  } catch (error) {
    console.error("Lỗi khi lấy danh sách phim:", error); // Ghi log lỗi
    res.status(500).json({ message: "Lỗi khi lấy danh sách phim", error }); // Phản hồi lỗi
  }
};
